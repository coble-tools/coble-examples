...
.save_figure <- function(ggp,dir,id,marker,width,height) {
  ...
  filepath <- file.path(dir,paste(x,'_',marker,'.pdf',sep=""))
  ggsave(
    file=filepath,ggp,
    width=width,
    height=height,
...
.plot_gene_expression <- function(markers, genes, mat, tsne_df, style, title, dir=NULL) {
  for (marker in markers) {
    ...
    if (is.null(dir)) {
      ...
      .save_figure(p,dir,title,marker,
      width=8.0,
      height=7.0)
    }
  }
...
.plot_heatmap_and_write_genes <- function(n_clust,tsne_df,mat,genes,dir) {
  ...
  write.table(sub_nor$gene_scores,file = paste0(dir,'_heatmap.tsv'),sep="\t",quote=F,row.names = F)
  pheatmap(sub_nor$sub_nor,
            cluster_rows = T,
            cluster_cols = T,
            fontsize_col=8,
            color=grey.colors(8),
            filename=paste0(dir,'_heatmap.png'))
}
# --------------------------------------------------
